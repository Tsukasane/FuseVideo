...
def DST(x): #Converts Scipy's DST output to Matlab's DST (scaling).
    X = scipy.fftpack.dst(x,type=1,axis=0)
    return X/2.0

def IDST(X): # Inverse DST. Python -> Matlab
    n = X.shape[0]
    x = np.real(scipy.fftpack.idst(X,type=1,axis=0))
    return x/(n+1.0)

def get_grads(im): # return the x and y gradients.
    [H,W] = im.shape
...
def get_laplacian(Dx,Dy): # return the laplacian
    [H,W] = Dx.shape
...
def poisson_solve(gx,gy,bnd): # convert to double:
...
def blit_images(im_top,im_back,scale_grad=1.0,mode='max'): # combine images using poission editing.
...
def contiguous_regions(mask): # return a list of (ind0, ind1) such that mask[ind0:ind1].all() is True and we cover all such regions
    in_region = None
    boundaries = []
...
def PoissonOut(bg_path, fg_path, out_path):
    ...
    fg_images = os.listdir(fg_path)

    for i_path in bg_images:
        bg_filename = bg_path+i_path
        fg_filename = fg_path+i_path

        im_src = cv2.imread(bg_filename).astype('float32')
        im_dst = cv2.imread(fg_filename).astype('float32')
        ...
        im0 = im_dst[:,:,0] > 220 #mask,取thing身上过亮的地方为true
        im_dst[im0,:] = im_src[im0,:] #过亮的地方直接用背景图代替
        #im_dst[~im0,:] = 50 
        im_dst = cv2.GaussianBlur(im_dst,(5,5),5)
        im_alpha = 0.35*im_dst + 0.65*im_src
        im_res = blit_images(im_src,im_dst)
        
        # imageio.imsave(f'{out_path}orig_{i_path}',im_src[:,:,::-1].astype('uint8'))
        # imageio.imsave(f'{out_path}alpha_{i_path}',im_alpha[:,:,::-1].astype('uint8'))
        imageio.imsave(f'{out_path}poisson_{i_path}',im_res[:,:,::-1].astype('uint8'))
        ...
        # imageio.imsave(f'{out_path}pL_{i_path}',im_poisson_L)
